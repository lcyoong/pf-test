<template>
    ...
    <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
        <MenuItems class="absolute right-0 z-10 mt-2 w-64 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div v-if="user.active_notifications.length > 0 && user.show_notification">
                <MenuItem v-for="notification in user.active_notifications" v-slot="{ active }" @click.prevent="read(notification)">
                    <div :class="[active ? 'bg-gray-100' : '', 'block px-4 py-2 text-sm text-gray-700']">
                        <span class="text-xs text-white bg-green-800 inline-block rounded px-2 py-0.5">{{ notification.type }}</span><br/>
...
function read(value)
{
    Inertia.post(route('impersonate.read', { user: props.user.id, notification: value }));
}
