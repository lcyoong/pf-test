<template>
  <Modal ref="modalRef" @close="localErrors = null">
    ...
    <div class="py-5">
      <form class="py-2 space-y-2" @submit.prevent="editUser">
        <div class="flex flex-col space-y-1">
          ...
          <ValidationError :error="localErrors?.editUser?.name"/>
        </div>
        <div class="flex flex-col space-y-1">
          ...
          <ValidationError :error="localErrors?.editUser?.phone"/>
        </div>
        <div class="flex flex-col space-y-1">
          ...
          <ValidationError :error="localErrors?.editUser?.email"/>
        </div>
        <div class="flex flex-col space-y-1">
        ...
        <button type="submit" class="flex-none justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Update</button>
      </form>
    </div>
  </Modal>
</template>
...
watch(() => props.user, (value) =>  {
  if(value) {
    ...
    form.show_notification = value.show_notification
  }
})
